...
model_path = "pinn_RC_AC_transient_500K.pt"
scaler_x_path = "pinn_RC_AC_transient_scaler_X_500K.pkl"
scaler_y_path = "pinn_RC_AC_transient_scaler_y_500K.pkl"
...
# Données d'extrapolation 
R = 1000
C = 1e-6
Vin = 5
frequencies = np.linspace( 1e6 , 5e6 , 300 )
...
plt.ylabel("Gain")
plt.title("Extrapolation des Gains (1 MHz à 5 MHz)")
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.show()
...
plt.title("Extrapolation des Phases (1 MHz à 5 MHz)")
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.show()
